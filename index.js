	...
	cumleKur fonksiyonuna aşağıdakileri uygulayın:
		...
		Yukarıdaki fonksiyonu foo("deneme") olarak çağırabiliriz, bu fonksiyon "deneme" stringini döndürecektir.
		 Aynı fonksiyonu foo(); şeklinde de çağırabiliriz. Bu durumda parametre olarak bir şey göndermediğimiz için
		  fonksiyon içindeki degisken değişkenine false değeri atanacaktır. Bu durumda foo(); fonksiyonunu çalıştırdığımız
		   false döndürür.
		)
...
/* (Oto test yok) cumleKur fonksiyonuna yalnızca 2 parametre göndererek "Hello World!" stringini elde edin,
 ...
 cumleKur("hello world", "");
/* (Oto test var) cumleKur fonksiyonuna 5 parametre göndererek "Ben iyi bir yazılımcı olacağım!" stringini elde edin, 
sonucu `bircumle` değişkenine atayın ve konsolde gözlemleyin */
var bircumle;
...
//	Sayfanın en üstünde global olarak tanımlanmış `cumleler` adında bir dizi bulunmaktadır. Bu dizinin içinde 
// en çok 5 en az 1 adet string bulunan diziler bulunmaktadır. Aşağıdaki görevlerde aksi belirtilmedikçe bu dizi kullanılacaktır.
...
/* 	GÖREV 1:
		cumlelereDonustur fonksiyonuna aşağıdaki yönergeleri uygulayın.
			...
			3. Dizinin içindeki her dizi elemanı 1 cümle oluşturacak şekilde uc uca eklenecek, kelimelerin aralarına
			 3. parametrede girilen ayraç yerleştirilecek;
...
/* GÖREV 2:
	...
	*/
	...
	function paragrafOlustur(cumledizi, cumleKurCB){
		let paragraf="";
		  for (let i=1; i<10; i+=2){
			paragraf += cumleKurCB(cumledizi[i]) ;
		  paragraf=paragraf.replaceAll(","," ");
...
/* 			3b.  Bir tavşan ve bir kirpi arkadaşlar sebzeler dizimizin peşine düştü.
 Tavşan => 🐇 , Kirpi=> 🦔 , Tavşanla kirpi sebzeleri ele geçirmek için bir plan kurdular.
  Tavşan diziye önden saldıracak, kirpi ise arkalarından dolaşacak. Varsayalım ki arkadaşların planları başarılı oldu. 
...
/* 	GÖREV 4:
		...
		sembolü bulunursa mesajı alan kişi bu sembolü "🙂" olarak görecek. Burdan yola çıkarak emojileriDonustur
		 fonksiyonuna aşağıdakileri uygulayın.
			...
			3. mesaj stringi emoji nesnesinde tanımlı olan tüm sembolleri istenilen değere dönüştürecek
			 (.replaceAll metodu kullanılacak)
				NOT: burada dikkat edilmesi gereken husus şudur; harfli sembolleri kullanıcı küçük
				 harf ve büyük harf olarak girebilir. 
 ...
 */
			...
			function emojileriDonustur(emojiString, emojiler){

				for (const key in emojiler){
					  
				emojiString = emojiString.replaceAll(key.toLowerCase(), emojiler[key]);
			  emojiString = emojiString.replaceAll(key.toUpperCase(), emojiler[key]);
				}
						  return emojiString;
			  }
